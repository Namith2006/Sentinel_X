...
def _normalize_image(raw: dict, filename: str) -> dict:
    ...
    if is_fake:
        if threshold == 40.0:
            reason = "Even though WhatsApp or Telegram compressed this image, we still found clear signs that it was created by AI."
            signs = [
                "Inconsistent compression artifacts detected",
                "Unnatural blending around facial features",
                "High-frequency synthetic noise patterns present"
            ]
        else:
            reason = "We detected unusual pixel patterns and fake details that usually mean an image was generated by AI."
            signs = [
                "Asymmetrical eye reflections or lighting",
                "Hair edges unnaturally blending into the background",
